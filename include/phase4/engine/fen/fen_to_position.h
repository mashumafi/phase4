...
private:
	static void ParseBoardState(string boardState, BoardState result) {
		...
		for (var rank = 0; rank < 8; rank++) {
			for (var file = 0; file < ranks[rank].Length; file++) {
				...
				if (char.IsLetter(c)) {
					...
					if (result.Walls != 0) {
						Debug.Assert(((1ul << position.GetHashCode()) & result.Walls) != 0);
					} else {
						result.Walls = WallOperations.SlideFromBB[file, rank];
					}
					position += new Position(1, 0);
				}
			}
	...
	static int ParseCurrentColor(string currentColor) {
		return currentColor == "w" ? Color.White : Color.Black;
	}

	static void ParseCastlingState(string castlingState, BoardState result) {
	...
	static void ParseEnPassantState(string enPassantState, BoardState result) {
		if (enPassantState != "-") {
	...
	static int ConvertToPiece(char c) {
	...
	static int ConvertToColor(char c) {
