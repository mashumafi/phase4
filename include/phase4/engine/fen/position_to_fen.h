		...
		for (int32_t rank = 7; rank >= 0; rank--) {
			...
			for (int32_t file = 7; file >= 0; file--) {
				...
				const PieceColor color = std::invoke([&]() -> PieceColor {
					if ((position.m_occupancyByColor[PieceColor::WHITE.get_raw_value()] & (1ul << fieldIndex)) != 0) {
						return PieceColor::WHITE;
					} else if ((position.m_occupancyByColor[PieceColor::BLACK.get_raw_value()] & (1ul << fieldIndex)) != 0) {
				...
				const bool hasWall = (position.m_walls & (1ul << fieldIndex)) != 0;
